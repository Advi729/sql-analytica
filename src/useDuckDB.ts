...
export function useDuckDB(): DuckDBState {
	...
	const loadCSV = useCallback(async (file: File) => {
		...
		if (!db || !conn) {
			throw new Error("DuckDB is not initialised.");
		}
		...
		try {
			...
			await conn.query(
				`CREATE TABLE "${tableName}" AS SELECT * FROM read_csv_auto('${safeName}')`,
			);
	...
	const runQuery = useCallback(async (sql: string) => {
		...
		try {
			...
			const columns: string[] = arrowTable.schema.fields.map(
				(field) => field.name,
			);
